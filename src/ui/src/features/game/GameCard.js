...
const cardStyle = {background: "#fff",borderRadius: "10px",boxShadow: "0 2px 8px rgba(0,0,0,0.07)",padding: "1.5rem",marginBottom: "1.2rem",display: "flex",alignItems: "center",justifyContent: "space-between",transition: "box-shadow 0.2s",};

const nameStyle = {fontSize: "1.6rem",fontWeight: 700,color: "#222",letterSpacing: "0.02em",display: "block",};
...
export default function GameCard({ id, game }) {
  ...
  return (
    <button
      key={game.id}
      style={{ ...clickableCardStyle, border: "none", outline: "none", width: "100%", textAlign: "left", padding: "0", background: "none", }}
      onClick={() => navigate(`/game/${game.id}`)}
      aria-label={`Open game ${game.name}`}
    >
      <div
        style={{ ...cardStyle, margin: 0, boxShadow: "none", padding: "1.5rem", }}
      >
